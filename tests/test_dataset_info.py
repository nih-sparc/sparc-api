...
def test_all_dataset_content(client):
    print("go")
    start = timer()
    ...
    for doi in found_doi_s:
        print(f'    "{doi.replace("https://doi.org/", "")}",')
    print("]")
...
def test_all_doi_dataset_search(client):
    for doi in doi_s:
        start = timer()
        print('Results for DOI: ', doi)
        r = client.get('/dataset_info_from_doi/', query_string={'doi': doi})
        # r = client.get('/search/contributors')
        end = timer()
        print('elapsed: ', end - start)
...
def test_doi_zdxd_84xz_dataset_search(client):
    # Quantified Morphology of the Pig Vagus Nerve
